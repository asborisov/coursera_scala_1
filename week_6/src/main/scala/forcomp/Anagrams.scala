...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.groupBy(c => c.toLower).map(p => p._1 -> p._2.length).toList.sortBy(_._1)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    x.map(p => (p._1, p._2 - yMap(p._1))).filter(_._2 > 0)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def loop(occurrences: Occurrences): List[Sentence] = {
      ...
      else combinations(occurrences)
        .flatMap(wordAnagrams)
        .flatMap(word => loop(subtract(occurrences, wordOccurrences(word))).map(word :: _))
    }
